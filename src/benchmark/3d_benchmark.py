...
class Benchmark3D:
    ...
    def create_prompt(self, observations, prior_structures=None) -> str:
        ...
        Args:
            observations: The observation data (string, list, or dict)
            prior_structures: List of previously generated structures to avoid repetition
        """

        # --- Helper: determine grid size ---
        def _get_grid_size(obs):
            if isinstance(obs, str):
                return int(len(obs) ** 0.5)
            elif isinstance(obs, list) and obs:
                if isinstance(obs[0], dict):
                    obs_data = obs[0].get('observation', [])
                    return len(obs_data) if obs_data else 3
                else:
                    obs_data = self._parse_observation(obs[0])
                    return getattr(obs_data, 'shape', [3])[0]
            else:
                obs_data = self._parse_observation(obs)
                return getattr(obs_data, 'shape', [3])[0]

        # --- Helper: format a single observation into text ---
        def _format_observation(obs) -> str:
            if isinstance(obs, dict):
                matrix = np.array(obs['observation'])
            else:
                matrix = self._parse_observation(obs)
            return "\n".join(" ".join(str(cell) for cell in row) for row in matrix)

        # --- Core logic ---
        grid_size = _get_grid_size(observations)
        max_height = self.metadata.get('max_height', 3)

        # Header
        prompt_parts = [
            f"You are given observations of a 3D structure made of unit blocks on a {grid_size}x{grid_size} grid.",
            "Each observation shows a view of the structure from a specific angle.",
            f"The maximum height of the structure is {max_height} layers.",
            "\nObservations (Top View - shows 1 if ANY layer has a block at that position):"
        ]

        # Observations
        if isinstance(observations, str):
            prompt_parts.append("\nTop view:\n" + _format_observation(observations))
        ...
        if prior_structures:
            ...
            for idx, struct in enumerate(prior_structures, 1):
                prior_text.append(f"\nAttempt {idx}:\n{struct.to_string()}")
            prompt_parts.extend(prior_text)
    ...
    """
        if prior_structures:
            task_text += f"""6. Generate a DIFFERENT valid structure from the {len(prior_structures)} prior attempts shown above
    ...
    1. Don't start with empty bottom layer
    2. Don't use commas (use spaces instead)
    3. Don't add unsupported blocks
    4. Don't exceed {max_height} layers
    """
...
def main():
    ...
    top_p = config.get('llm', {}).get('top_p', 0.4)
