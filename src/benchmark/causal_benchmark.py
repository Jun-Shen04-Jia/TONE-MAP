...
from src.generate_dataset.causal_dataset import PerturbationObservation, CausalDatasetGenerator
...
class CausalBenchmarkEnhanced:
    ...
    def create_prompt(self, observations: list, prior_hypotheses: list = None) -> str:
        """
        Create an EmotionPrompt-enhanced instruction for inferring a causal DAG from perturbation observations.
        - Integrates tone control and affective stimuli to boost careful reasoning and exploration diversity.
        - Preserves strict structural constraints and one-line output formatting for deterministic parsing.

        Args:
            observations: List[Dict] with key "string" per observation (already canonicalized upstream).
            prior_hypotheses: List[CausalGraph] previously proposed (each with .edges as iterable of (src, dst)).
        ...
        obs_block = "\n".join(obs["string"] for obs in observations) if observations else "None"

        # Prior hypotheses as edge lists (to enforce diversity without overfitting the model’s output format)
        if prior_hypotheses:
            prior_lines = []
            for h in prior_hypotheses:
                edges = [f"{s}->{d}" for (s, d) in getattr(h, "edges", [])]
                prior_lines.append("Graph: " + (", ".join(edges) if edges else "No edges"))
            prior_block = "\n".join(prior_lines)
        else:
            prior_block = "None"

        # Optional max-edge constraint
        constraint_info = ""
        if getattr(self, "max_edges", None) is not None:
            constraint_info = f"\nConstraint: The graph should have at most {self.max_edges} edges."

        # ————— Tone and affective stimuli —————
        tone_variants = {
            "polite": "Please analyze the observations and infer a consistent causal graph.",
            "neutral": "Analyze the observations and infer a consistent causal graph.",
            "direct": "Infer a causal graph consistent with the observations. Focus on correctness.",
            "assertive": "Carefully infer a causal graph consistent with the observations. Precision and consistency are critical."
        }
        tone = getattr(self, "tone", "assertive")
        tone_prefix = tone_variants.get(tone, tone_variants["assertive"])

        emotional_pool = [
            "This task is important for advancing our research—take pride in being thorough.",
            "Your precise reasoning and careful validation will directly impact scientific rigor.",
            "Treat this as a mission-critical analysis—accuracy and completeness are essential.",
            "Your diligence in exploring valid alternatives is key to robust scientific inference.",
            "Think deeply, verify carefully, and ensure your proposal withstands logical scrutiny."
        ]
        emotional_line = getattr(self, "emotion_line", emotional_pool[0])

        # ————— Prompt body —————
        prompt = f"""
        {tone_prefix} {emotional_line}
        ...
        The graph must be acyclic, use only the listed nodes, and avoid self-loops.

        Diversity rule:
        - A "diverse" graph is any valid graph whose edge set is NOT identical to any prior prediction.
        - Prefer diverse graphs when multiple explanations are consistent with the data.

        Validation checklist:
        - Edges imply descendant relationships consistent with all observation rows.
        - No cycles; no self-loops; only nodes from the provided list are used.
        - Respect any edge-count constraint if specified.
        - If multiple DAGs fit equally well, choose one that is structurally distinct from priors.
        ...
        3) Do not include explanations or extra text—only the single required line.
        """
...
def main():
    ...
    temperature = config.get('llm', {}).get('temperature', 0.6)
