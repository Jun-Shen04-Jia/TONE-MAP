...
from typing import List, Dict, Set, Tuple, Optional, Iterable, Dict, Any
...
from modules.llm_interface import LLMInterface, OpenRouterLLM, OpenAILLM, AnthropicLLM, GeminiLLM
import sys
import os
import sys
import os
...
# 现在可以直接导入
from src.generate_dataset.boolean_dataset import BooleanExpression, BooleanObservation
...
class BooleanBenchmarkRefined:
    ...
    def create_prompt(
        ...
        """Create a prompt for Boolean expression induction with structural uniqueness constraints."""

        # --- Validation & defaults ---
        variables: List[str] = getattr(self, 'variables', [])
        operators: List[str] = getattr(self, 'operators', [])
        max_depth: int = getattr(self, 'max_depth', 4)

        if not variables:
            raise ValueError("Variables list is empty. Provide at least one variable.")
        if not operators:
            raise ValueError("Operators list is empty. Provide at least one operator.")
        if not isinstance(observations, Iterable) or not observations:
            raise ValueError("Observations must be a non-empty list of BooleanObservation.")

        # --- Helpers ---
        def _format_observations(obs_list: List['BooleanObservation']) -> str:
            """Each observation serialized as 'input -> output' lines via obs.to_string()."""
            try:
                return "\n".join(obs.to_string() for obs in obs_list)
            except Exception as e:
                raise ValueError(f"Failed to format observations: {e}")

        def _format_prior(hypotheses: List[str]) -> str:
            """Prior expressions formatted as 'Expression: ...' lines; 'None' if empty."""
            if not hypotheses:
                return "None"
            lines = []
            for h in hypotheses:
                h_clean = str(h).strip()
                if not h_clean:
                    # Skip empty lines to avoid false duplicates
                    continue
                # Normalize whitespace inside operators for downstream equivalence checks
                lines.append(f"Expression: {h_clean}")
            return "\n".join(lines) if lines else "None"

        def _operator_descriptions(ops: Iterable[str]) -> str:
            """Human-readable operator descriptions, ordered as provided."""
            mapping = {
                'AND': "AND (conjunction)",
                'OR': "OR (disjunction)",
                'NOT': "NOT (negation)",
            ...
            unknown = [op for op in ops if op not in mapping]
            if unknown:
                raise ValueError(f"Unsupported operators: {', '.join(unknown)}")
            return ", ".join(mapping[op] for op in ops)

        def _equivalence_rules(mech: Dict[str, Any]) -> str:
            """Build equivalence rules block from mechanistic options with explicit examples."""
            apply_commutativity = mech.get('apply_commutativity', True)
            apply_idempotence = mech.get('apply_idempotence_and_or', True)
            flatten_associativity = mech.get('flatten_associativity', True)

            rules: List[str] = []

            if apply_commutativity:
                rules.append(
                    "- Commutativity: reorderings are the SAME\n"
                    "          e.g., x AND y = y AND x; x OR y = y OR x."
                )
            else:
                rules.append(
                    "- No commutativity: reorderings are DIFFERENT\n"
                    "          e.g., x AND y ≠ y AND x."
                )

            if apply_idempotence:
                rules.append(
                    "- Idempotence: duplicates collapse under AND/OR\n"
                    "          e.g., x AND x = x; x OR x = x."
                )
            else:
                rules.append(
                    "- No idempotence: duplicates are DIFFERENT\n"
                    "          e.g., x AND x ≠ x."
                )

            if flatten_associativity:
                rules.append(
                    "- Associativity flattening: cascades of the SAME operator are the SAME regardless of parentheses\n"
                    "          e.g., (x AND y) AND z = x AND (y AND z) = x AND y AND z."
                )
            else:
                rules.append(
            ...
            rules.append(
                "- No distributivity, absorption, or De Morgan normalization across different operators:\n"
                "          mixing operators keeps expressions DIFFERENT\n"
                "          e.g., (x AND y) OR z ≠ x AND (y OR z)."
            )

            # Indent the lines to visually align under the list in the prompt
            return "\n            ".join(rules)

        # --- Build blocks ---
        obs_block = _format_observations(observations)
        prior_block = _format_prior(prior_hypotheses)
        operators_str = _operator_descriptions(operators)

        mech_opts = getattr(self, 'complete_dataset', {}).get('metadata', {}).get('mechanistic_opts', {})
        rules_text = _equivalence_rules(mech_opts)

        # --- Prompt assembly ---
        header = (
            f"You are given partial observations of a Boolean function with variables: {', '.join(variables)}\n\n"
            f"Allowed operators: {operators_str}\n\n"
        ...
        task_spec = (
            ...
            "Requirements:\n"
            f"1. Use ONLY the variables: {', '.join(variables)}\n"
            f"2. Use ONLY these operators: {', '.join(operators)}\n"
            "3. The expression must match all given observations\n"
            "4. Structural uniqueness is judged by these rules:\n"
            f"            {rules_text}\n"
            f"5. Expression depth should be at most {max_depth} levels of nesting\n"
        ...
        prompt = header + task_spec + output_format
        return prompt
...
def main():
    ...
    llm_type = config.get('llm', {}).get('type', 'gemini')
    ...
    top_p = config.get('llm', {}).get('top_p', 0.8)
